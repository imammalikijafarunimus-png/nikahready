...
const PANDANGAN_FIELDS: Array<{
...
}> = [
  {
    ...
    icon: '💬',
    placeholder: 'Apa arti pernikahan bagimu? Apa yang kamu pahami tentang tujuan berumah tangga dalam Islam?',
  ...
  },
  {
    ...
    icon: '👩‍💼',
    placeholder: 'Bagaimana pandanganmu jika istri ingin berkarir? Apa batasan dan pertimbanganmu?',
  ...
  },
  {
    ...
    icon: '👶',
    placeholder: 'Berapa jumlah anak yang ideal menurutmu? Bagaimana pandanganmu tentang KB?',
  ...
  },
  {
    ...
    icon: '🧒',
    placeholder: 'Bagaimana cara mendidik anak yang kamu yakini? Sekolah formal, pesantren, atau homeschooling?',
  ...
  },
  {
    ...
    icon: '🏠',
    placeholder: 'Setelah menikah, bagaimana rencanamu soal tempat tinggal? Bagaimana mengelola hubungan dengan mertua?',
...
export function Step13_PandanganIsu() {
  ...
  function update<K extends keyof PandanganIsu>(field: K, value: PandanganIsu[K]) {
    dispatch({
      type: 'UPDATE_FIELD',
  ...
  return (
    <div className="space-y-5">
      {/* Tip Card Navy Pattern */}
      <div className="flex gap-3 p-3 rounded-xl bg-navy-900/60 border border-navy-800">
        <span className="text-lg flex-shrink-0">💬</span>
        <p className="text-xs text-navy-400 leading-relaxed">
          Bagian ini membahas isu-isu yang sering menjadi sumber <strong className="text-white">gesekan dalam rumah tangga</strong>. Kejujuran di sini lebih baik daripada kejutan setelah menikah.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 px-1">
        <div className="flex-1 h-1 bg-navy-800 rounded-full">
          <div
            ...
            style={{ width: `${Math.round((filledCount / PANDANGAN_FIELDS.length) * 100)}%` }}
          />
        </div>
        <span className="text-[10px] text-navy-500 uppercase tracking-wider font-medium">
          {filledCount}/{PANDANGAN_FIELDS.length} Selesai
        </span>
      </div>
      ...
      <SectionCard variant="highlight">
        <div className="text-center space-y-1 py-1">
          ...
          <p className="text-[10px] text-navy-400 italic">
            &ldquo;Dan orang-orang yang berkata: Ya Tuhan kami, anugerahkanlah kepada kami pasangan kami&hellip;&rdquo;
            <br />
